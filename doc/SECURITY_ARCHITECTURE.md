...
On an SELinux system, every process and object has an SELinux security context which identifies
the object's security attributes. Policy rules authorize interactions between a source context and
a target context for specific object classes and permissions. Access is denied unless the loaded
policy allows it and all applicable constraints are satisfied.
...
A domain is a process type with the `domain` attribute. One or more processes may share a domain,
which forms their logical security boundary. Separate domains allow policy to grant each domain
the rules required for its function and limit the effect of compromise.

Domain type names normally use the `_t` suffix and correspond to a service name or process name.
For example, the `sshd` service runs in the `sshd_t` domain. The suffix is a naming convention;
membership in the `domain` attribute is what makes a type a domain.
...
Process transitions must be explicit and constrained. The core policy uses `neverallow` rules to
prevent transitions to types that are not domains.

### Object Types

Object types, not to be confused with object classes, label resources accessed by domains. They
...
Types should separate objects with different readers, writers, trust levels, lifecycles, or
transition behavior. A service commonly has distinct types for executable code, configuration,
persistent state, logs, temporary data, and runtime objects. Network, device, filesystem, and IPC
types similarly distinguish resources that Linux discretionary access control may otherwise treat
alike.

### Type Attributes

Attributes group types that share a security property. Core examples include `domain`,
...
1. A Linux account is mapped to an SELinux user.
...
the process and entry-point transition. Tools and domains for `newrole`, `sudo`, `su`, login, and
similar mechanisms provide controlled transition paths.
...
scoped authority to trusted domains that assign category sets to new processes. This supports
