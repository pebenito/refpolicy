...
discretionary access control and application security. The policy seeks to limit the authority and
...
IPC object should not imply authority to communicate with every domain that can access the
...
- the loaded policy and policy module store;
- SELinux configuration, labeling rules, and policy sources installed on the system;
- kernel interfaces that control enforcement or expose security-sensitive state;
- authentication, authorization, audit, logging, and system initialization services; and
- executables, libraries, devices, and persistent state required by the trusted computing base.

Authority to load policy, change enforcement state, relabel protected objects, administer security
services, or modify their configuration should be confined to explicitly authorized administrative
and system domains.

In particular, authority to load policy, change Boolean values, or set enforcement mode should be
limited to the domains identified for those functions. Direct kernel module loading and powerful
capabilities such as `CAP_SYS_MODULE`, `CAP_SYS_ADMIN`, `CAP_SYS_RAWIO`, and `CAP_NET_ADMIN` should
be restricted to domains with a documented need.
...
not by itself grant unrestricted SELinux authority.

- Ordinary users should operate in unprivileged roles and domains by default.
- Administrative authority should be entered through explicit, authorized role and domain
  transitions.
...
- A process should change its own security context only when the transition is expected and
  constrained. Dynamic transitions from the kernel during system initialization should not permit
  targets other than the kernel or init domains.
...
1. Does the change grant a domain authority beyond its documented function?
...
5. Could an unprivileged user, role, or compromised service use the change to gain administrative
   or unrelated service authority?
